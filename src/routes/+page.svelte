...
<div class="w-5/6 mx-auto mt-2">
	<form action="?/tasks" method="POST">
		...
		<div class="block self-center mb-4">
			...
			<input type="radio" name="months" value="1" required="required" />
			<label class="ml-2 mr-1">2M</label>
			<input type="radio" name="months" value="2" required="required" />
			<label class="ml-2 mr-1">3M</label>
			<input type="radio" name="months" value="3" required="required" />
		</div>
	...
	{#if tasks.success}
			...
			<div>
				...
				<p>
					<span class="font-medium">Tareas sin clasificar: </span><span class="text-indigo-500"
						>{tasks.unclasificated.length}</span
					>
				</p>
			</div>
		...
		<div class="flex justify-start mt-2 flex-wrap">
			{#each tasks.unclasificated as task}
		...
		<div class="mx-auto w-full flex justify-center my-10">
			<table class="w-full">
				...
				<tbody>
					{#each Object.entries(tasks.cal) as [key, week]}
						<tr class={week.week === tasks.actual ? '!bg-indigo-100' : ''}>
